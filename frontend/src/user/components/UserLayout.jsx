  ...
  <motion.div className="fixed inset-0 z-[9999] flex items-center justify-center px-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
    ...
    <motion.div
      ...
      style={{
        ...
        boxShadow: "0 0 60px #00e5ff08, 0 20px 60px #00000099",
      }}
    >
      <div className="h-[2px] bg-gradient-to-r from-[#00e5ff] via-[#10d990] to-transparent" />
      <div className="p-7 text-center">
        <motion.div className="w-14 h-14 rounded-2xl mx-auto mb-5 flex items-center justify-center border border-[#00e5ff30] bg-[#00e5ff08]" initial={{ rotate: -10, scale: 0.8 }} animate={{ rotate: 0, scale: 1 }}>
          <LogOut className="w-6 h-6 text-[#00e5ff]" />
        </motion.div>
        <h3 className="text-lg font-black text-[#e8f4ff] mb-2 uppercase" style={{ fontFamily: "var(--font-display)" }}>
          Sign Out
        </h3>
...
const UserNavbar = () => {
  ...
  return (
    <>
      <nav className="fixed top-0 left-0 right-0 z-50 border-b border-[#00e5ff18] bg-[#060d18]/92 backdrop-blur-xl" style={{ boxShadow: "0 4px 30px #00000066" }}>
          ...
          <div className="flex items-center justify-between h-16">
            <Link to="/dashboard" className="flex items-center gap-3 group select-none">
              <div className="relative w-9 h-9">
                <motion.div className="absolute inset-0" animate={{ rotate: 360 }} transition={{ duration: 20, repeat: Infinity, ease: "linear" }}>
                  <svg viewBox="0 0 36 36" fill="none">
                    <path d="M18 2 L32 10 L32 26 L18 34 L4 26 L4 10 Z" stroke="#00e5ff" strokeWidth="1.5" fill="none" opacity="0.6" />
                  </svg>
                </motion.div>
              ...
              <div className="flex flex-col leading-none">
                ...
                <span className="text-[10px] font-light tracking-[0.3em] text-[#8ab4d4] uppercase" style={{ fontFamily: "var(--font-mono)" }}>
                  IQ / User
                </span>
              </div>
            </Link>
            ...
            <div className="flex items-center gap-3">
              <div className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-[#00e5ff30] bg-[#00e5ff08]">
                ...
                <span className="text-[10px] text-[#00e5ff] uppercase tracking-widest font-bold" style={{ fontFamily: "var(--font-mono)" }}>
                  User
                </span>
              </div>
              <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.96 }} onClick={() => setShowLogout(true)} className="flex items-center gap-2 px-2.5 py-1.5 rounded-full border border-[#1a2a4a] bg-[#04080f] hover:border-[#00e5ff33] transition-all">
                <div className="w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-black text-[#020408]" style={{ background: "linear-gradient(135deg,#00e5ff,#10d990)", fontFamily: "var(--font-display)", boxShadow: "0 0 14px #00e5ff55" }}>
                  {initials}
                </div>
                <span className="hidden sm:block text-[10px] text-[#8ab4d4] uppercase tracking-wider" style={{ fontFamily: "var(--font-mono)" }}>
                  {user?.name?.split(" ")[0]}
                </span>
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="fixed top-16 left-4 right-4 z-40 rounded-2xl border border-[#00e5ff18] bg-[#060d18] p-3 md:hidden" style={{ boxShadow: "0 20px 60px #00000099" }}>
            {NAV.map(({ icon: Icon, label, path }) => (
              <Link key={path} to={path} onClick={() => setMobileOpen(false)}>
                <div className={`flex items-center gap-3 px-4 py-3 rounded-xl text-[11px] uppercase tracking-wider font-bold mb-0.5 transition-all ${location.pathname === path ? "bg-[#00e5ff15] text-[#00e5ff]" : "text-[#3d6080] hover:text-[#8ab4d4]"}`} style={{ fontFamily: "var(--font-mono)" }}>
    ...
    <div className="max-w-screen-xl mx-auto flex items-center justify-between gap-3">
      <div className="flex items-center gap-3">
        ...
        <span className="text-[11px] text-[#3d6080]" style={{ fontFamily: "var(--font-mono)" }}>
          PulseIQ <span className="text-[#1a3a6b]">/ v2</span>
        </span>
      </div>

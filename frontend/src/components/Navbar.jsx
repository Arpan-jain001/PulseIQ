...
const Logo = () => (
  <Link to="/" className="flex items-center gap-3 group select-none">
    <div className="relative w-9 h-9">
      <motion.div className="absolute inset-0" animate={{ rotate: 360 }} transition={{ duration: 20, repeat: Infinity, ease: "linear" }}>
        <svg viewBox="0 0 36 36" fill="none">
          <path d="M18 2 L32 10 L32 26 L18 34 L4 26 L4 10 Z" stroke="#00e5ff" strokeWidth="1.5" fill="none" opacity="0.5" />
        </svg>
      </motion.div>
    ...
    <div className="flex flex-col leading-none">
      <span className="text-sm font-black tracking-[0.2em] text-[#00e5ff] uppercase" style={{ fontFamily: "var(--font-display)" }}>
        Pulse
      </span>
      <span className="text-[10px] font-light tracking-[0.4em] text-[#8ab4d4] uppercase" style={{ fontFamily: "var(--font-mono)" }}>
        IQ / v2
      </span>
    </div>
  </Link>
...
const LogoutModal = ({ onConfirm, onCancel }) => (
  <AnimatePresence>
    <motion.div className="fixed inset-0 z-[9999] flex items-center justify-center px-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      <motion.div className="absolute inset-0 bg-black/70 backdrop-blur-sm" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onCancel} />
      <motion.div
      ...
      >
        {["top-0 left-0", "top-0 right-0", "bottom-0 left-0", "bottom-0 right-0"].map((position, index) => (
          <div key={index} className={`absolute ${position} w-6 h-6 pointer-events-none`}>
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
          <p className="text-[10px] text-[#00e5ff] uppercase tracking-[0.2em] mb-2" style={{ fontFamily: "var(--font-mono)" }}>
            PulseIQ Session
          </p>
          <h3 className="text-lg font-black text-[#e8f4ff] mb-2 uppercase tracking-wide" style={{ fontFamily: "var(--font-display)" }}>
            Sign Out
          </h3>
          <p className="text-xs text-[#3d6080] leading-relaxed mb-7" style={{ fontFamily: "var(--font-mono)" }}>
            Are you sure you want to end your current session? Your data and analytics remain safe.
          </p>
        </motion.div>

        <motion.div className="flex gap-3" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
          <motion.button
...
const UserMenu = ({ user, onLogoutClick }) => {
  ...
  const initials = user?.name ? user.name.split(" ").map((name) => name[0]).join("").toUpperCase().slice(0, 2) : "U";
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <span className="hidden sm:block text-[10px] text-[#8ab4d4] uppercase tracking-wider max-w-[80px] truncate" style={{ fontFamily: "var(--font-mono)" }}>
          {user?.name?.split(" ")[0]}
        </span>
        ...
        {open && (
          ...
          >
            <div className="px-4 py-3 border-b border-[#0d2140]">
              <p className="text-[11px] font-bold text-[#e8f4ff] truncate" style={{ fontFamily: "var(--font-mono)" }}>
                {user?.name}
              </p>
              <p className="text-[9px] uppercase tracking-widest mt-0.5" style={{ color: roleColor, fontFamily: "var(--font-mono)" }}>
                {user?.role?.replace("_", " ")}
              </p>
            </div>
    ...
    <>
      <motion.nav
        ...
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${
          scrolled ? "py-3 px-5 md:px-10 glass-dark border-b border-[#00e5ff15] shadow-[0_0_40px_#00000080]" : "py-5 px-6 md:px-10 bg-transparent"
        }`}
      >
        {scrolled && <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-[#00e5ff44] to-transparent" />}
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-1 bg-[#04080f]/80 backdrop-blur-sm rounded-full px-2 py-1.5 border border-[#0d2140]">
            {NAV_LINKS.map((item) => (
              ...
              >
                ...
                <motion.div className="absolute inset-0 rounded-full bg-[#00e5ff08] border border-[#00e5ff20]" initial={{ opacity: 0 }} whileHover={{ opacity: 1 }} transition={{ duration: 0.2 }} />
              </motion.a>
            ))}
          </div>
          ...
          <div className="hidden md:flex items-center gap-3">
            {user ? (
              ...
              <>
                <Link to="/login">
                  <motion.button whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} className="px-5 py-2.5 text-xs font-medium text-[#8ab4d4] hover:text-[#00e5ff] transition-colors uppercase tracking-widest" style={{ fontFamily: "var(--font-mono)" }}>
                    Log In
                  </motion.button>
                </Link>
                <Link to="/signup">
              ...
              {user ? (
                <>
                  <div className="flex items-center gap-3 px-4 py-2 mb-2">
                    <div className="w-8 h-8 rounded-full flex items-center justify-center text-[11px] font-black text-[#020408] flex-shrink-0" style={{ background: "#00e5ff", fontFamily: "var(--font-display)" }}>
                      {user.name?.split(" ").map((name) => name[0]).join("").toUpperCase().slice(0, 2)}
                    </div>
                    <div>
                      <p className="text-[11px] text-[#e8f4ff] font-bold" style={{ fontFamily: "var(--font-mono)" }}>
                        {user.name}
                      </p>
